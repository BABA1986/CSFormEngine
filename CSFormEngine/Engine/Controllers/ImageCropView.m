...
{
    self =  [super initWithNibName: @"ImageCropView" bundle: nil];
    if (self)
    {
...
- (void)viewDidLoad
{
    ...
    //confirm button background color or text color
    confrmButton.backgroundColor= [UIColor blueColor];
    [confrmButton setTitleColor: [UIColor whiteColor] forState: UIControlStateNormal];
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    {
        // Initialization code
        self.color = [UIColor colorWithRed:255.0/255.0 green:255.0/255.0 blue:255.0/255.0 alpha:1];
        self.opaque = NO;
    }
    return self;
}
...
- (void)setControlPosition:(ControlPosition)controlPosition
{
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGFloat lMinX = CGRectGetMinX(rect);
    CGFloat lMinY = CGRectGetMinY(rect);
    
    CGFloat lMidX = CGRectGetMidX(rect);
    CGFloat lMidY = CGRectGetMidY(rect);
    
    CGFloat lMaxX = CGRectGetMaxX(rect);
    CGFloat lMaxY = CGRectGetMaxY(rect);
    
    CGContextMoveToPoint(context, lMinX, lMidY/2);
    CGContextAddLineToPoint(context, lMinX, lMinY);
    CGContextAddLineToPoint(context, lMidX/2, lMinY);
    
    CGContextMoveToPoint(context, lMaxX*0.75, lMinY);
    CGContextAddLineToPoint(context, lMaxX, lMinY);
    CGContextAddLineToPoint(context, lMaxX, lMidY/2);
    
    CGContextMoveToPoint(context, lMinX, lMaxY*0.75);
    CGContextAddLineToPoint(context, lMinX, lMaxY);
    CGContextAddLineToPoint(context, lMidX/2, lMaxY);
    
    CGContextMoveToPoint(context, lMaxX*0.75, lMaxY);
    CGContextAddLineToPoint(context, lMaxX, lMaxY);
    CGContextAddLineToPoint(context, lMaxX, lMaxY*0.75);
    
    CGContextStrokePath(context);
    
    // Circle draw here
    CGContextRef ctx = UIGraphicsGetCurrentContext();
    CGRect border = CGRectInset(rect, 1, 1);
    CGContextAddEllipseInRect(ctx, border);
...
- (void)initViews
{
    ...
    topLeftPoint = [self createControlPointAt:SquareCGRectAtCenter(centerInView.x - initialClearAreaSize,
                                                                   ...
                                                                   controlPointSize)];
    topLeftPoint.controlPosition = EControlPositionTopLeft;
    
    bottomLeftPoint = [self createControlPointAt:SquareCGRectAtCenter(centerInView.x - initialClearAreaSize,
                                                                      centerInView.y + initialClearAreaSize,
                                                                      controlPointSize)];
    bottomLeftPoint.controlPosition = EControlPositionBottomLeft;
    ...
    bottomRightPoint.controlPosition = EControlPositionBottomRight;
    
    topRightPoint = [self createControlPointAt:SquareCGRectAtCenter(centerInView.x + initialClearAreaSize,
                                                                    centerInView.y - initialClearAreaSize, controlPointSize)];
    topRightPoint.controlPosition = EControlPositionTopRight;
    ...
    cropAreaView.layer.cornerRadius= 5;
    ...
    [self.shadeView.blurredImageView setNeedsDisplay];
    [cropAreaView setNeedsDisplay];
...
- (void)handleDragTopLeft:(CGPoint)dragLocation
{
    ...
    //No dragging from top left if its point is on image frame boundary
    if((dragPoint.topLeftCenter.x == imageFrameInView.origin.x || dragPoint.topLeftCenter.y == imageFrameInView.origin.y) && (disp.width < 0 && disp.height <0))
        return;
    
    //To check dispalcement is not more than image frame width
    if(dragPoint.topLeftCenter.x + disp.width < imageFrameInView.origin.x)
    {
        disp.width = - (fabs(disp.width) -  fabs(imageFrameInView.origin.x - (dragPoint.topLeftCenter.x + disp.width)));
        disp.height =  disp.width;
    }
    //To check displacement is not more than image frame height
    if(dragPoint.topLeftCenter.y + disp.height < imageFrameInView.origin.y)
    {
        disp.height = - (fabs(disp.height) -  fabs(imageFrameInView.origin.y - (dragPoint.topLeftCenter.y + disp.height)));
...
- (void)handleDragBottomLeft:(CGPoint)dragLocation {
    ...
    //No dragging from bottom left if its point is on image frame boundary
    if((dragPoint.bottomLeftCenter.x == imageFrameInView.origin.x || dragPoint.bottomLeftCenter.y == imageFrameInView.origin.y + imageFrameInView.size.height) && (disp.width < 0 && disp.height > 0))
        return;
    
    //To check dispalcement is not more than image frame width
    if(dragPoint.bottomLeftCenter.x + disp.width < imageFrameInView.origin.x)
    {
        disp.width = - (fabs(disp.width) - fabs(imageFrameInView.origin.x - (dragPoint.topLeftCenter.x + disp.width)));
        disp.height = - disp.width;
    }
    //To check displacement is not more than image frame height
    if (dragPoint.bottomLeftCenter.y + disp.height > imageFrameInView.origin.y + imageFrameInView.size.height)
    {
        disp.height -= (dragPoint.bottomLeftCenter.y + disp.height) - (imageFrameInView.origin.y + imageFrameInView.size.height);
        disp.width = - disp.height;
    }
...
- (void)handleDragBottomRight:(CGPoint)dragLocation {
    ...
    //No dragging from bottom right if its point is on image frame boundary
    if((dragPoint.bottomRightCenter.x == imageFrameInView.origin.x + imageFrameInView.size.width || dragPoint.bottomRightCenter.y == imageFrameInView.origin.y + imageFrameInView.size.height) && (disp.width > 0 && disp.height > 0))
        return;
    
    //To check dispalcement is not more than image frame width
    if(dragPoint.bottomRightCenter.x + disp.width > imageFrameInView.origin.x + imageFrameInView.size.width)
    {
        disp.width -= (dragPoint.bottomRightCenter.x + disp.width) - (imageFrameInView.origin.x + imageFrameInView.size.width);
        disp.height = disp.width;
    }
    //To check displacement is not more than image frame height
    if (dragPoint.bottomRightCenter.y + disp.height > imageFrameInView.origin.y + imageFrameInView.size.height)
    {
        disp.height -= (dragPoint.bottomRightCenter.y + disp.height) - (imageFrameInView.origin.y + imageFrameInView.size.height);
        disp.width = disp.height;
    }
...
- (void)handleDragTopRight:(CGPoint)dragLocation {
    ...
    //No dragging from top right if its point is on image frame boundary
    if((dragPoint.topRightCenter.x == imageFrameInView.origin.x + imageFrameInView.size.width|| dragPoint.topRightCenter.y == imageFrameInView.origin.y) && (disp.width > 0 && disp.height < 0))
        return;
    
    //To check dispalcement is not more than image frame width
    if(dragPoint.topRightCenter.x + disp.width > imageFrameInView.origin.x + imageFrameInView.size.width)
    {
        disp.width -= (dragPoint.topRightCenter.x + disp.width) - (imageFrameInView.origin.x + imageFrameInView.size.width);
        disp.height = - disp.width;
    }
    //To check displacement is not more than image frame height
    if (dragPoint.topRightCenter.y + disp.height < imageFrameInView.origin.y)
    {
        disp.height = - (fabs(disp.height) - fabs(imageFrameInView.origin.y - (dragPoint.topLeftCenter.y + disp.height)));;
        disp.width = - disp.height;
    }
