...
- (void)drawRect:(CGRect)rect
{
    ...
    UIImage* lImage = [UIImage imageNamed: @"imgDrpDown@2x.png"];
    CGRect lRect = rect;
    lRect.size.width = 15;
    lRect.size.height = 15;
    lRect.origin.x = CGRectGetWidth(rect) - 20;
    lRect.origin.y = (CGRectGetHeight(rect) - 15)/2;
...
- (void)initialise
{
    ...
    CGRect lBoundFrame = lBaseView.frame;
    lBoundFrame = CGRectInset(lBoundFrame, 0, kOffset);
    mBoundFrame = lBoundFrame;
...
//    [self.selectedContentLabel sizeToFit];
//    self.selectedContentLabel.font = kDefautFont(kFontSize);
    self.selectedContentLabel.text = self.selectorTitle;
    self.selectedContentLabel.textColor = [UIColor blackColor];
    self.selectedContentLabel.numberOfLines = 0;
...
- (void)makeEnable:(BOOL)enable
{
    ...
    UIColor* lTextCOlor = enable ? [UIColor blackColor] : [UIColor grayColor];
...
{
    //[self hideDrop];
//    if(LOGFLG) NSLog(@"touchesCancelled");
}
...
//    lCell.textLabel.font = kDefautFont(kFontSize);
    ...
    if (mAllowMultiSelection)
    {
        ...
        lCell.accessoryType = lSelected ? UITableViewCellAccessoryCheckmark : UITableViewCellAccessoryNone;
    }
    
    if ([self.selectedContentLabel.text isEqualToString:lText] && !mAllowMultiSelection)
    {
        lCell.backgroundColor = [UIColor lightGrayColor];
